...
**候选阶梯（可选）**：请求携带 `ladder_size`（整数，2–5）时，响应附加 `ladder` 字段，
供复核员在"当前高可信证据被否定"的假设下查看最接近的其他正文：

- 从**所有**完整一致覆盖中，按每份**不同正文**可达的最大（总权重，片段数）计分；
  同一正文的多个覆盖只汇入其最优得分，**不重复占位**；
- 各阶按得分降序排列，同分再按**无符号正文升序**，取前 `ladder_size` 份；
- 每份正文的见证取其同分覆盖中片段编号升序列表字典序最小者（权重为正时即全相容集），
  `witness_fragment_ids` 按编号升序给出；
- 每阶 `diff_from_prev` 标明与上一阶正文**首次不同的字节位置**（零起算，第一阶为 `null`）；
- `exhausted=true` 表示已列出全部现存不同正文（不足请求级数时必然已穷尽）；
- `IMPOSSIBLE` 时阶梯为空（`entries: []`）且已穷尽，**不伪造候选**；
- 未携带 `ladder_size` 时，请求与响应与原契约**完全一致**（无 `ladder` 键）。
...
可选字段 `ladder_size`（2–5）启用候选阶梯，响应附加 `ladder`：
...
`ladder_size` 非法（非整数或超出 2–5）同样返回 422，定位 `["body", "ladder_size"]`。
...
1. `pytest`：求解器与 API 共 52 项测试，覆盖高权片段互斥、等分正文、缺口、冲突致不可行、
   等权双正文的无符号字节序、同权重按片段数决胜、非法输入 422 与字段定位，
   以及候选阶梯的得分降序、同分字节序、同正文去重、首异字节、穷尽标记、
   IMPOSSIBLE 空阶梯、28 片段大规模场景与 `ladder_size` 校验；
2. `npm run build`：TypeScript 严格类型检查 + Vite 生产构建；
3. `scripts/smoke_api.py`：容器内启动真实 uvicorn，发起 HTTP 冒烟（健康检查、静态首页、
   UNIQUE / AMBIGUOUS / IMPOSSIBLE、候选阶梯与 422 字段定位）。
...
- **输入区**：内联编辑编号/偏移/载荷/权重（2–28 行），或"导入 JSON"批量导入；
  字段级红框即时提示；样例芯片一键载入（等分正文 / 高权互斥 / 正文歧义 / 缺口 / 冲突致不可行 / 候选阶梯）。
...
- **候选阶梯**：裁决区内可选 2–5 级并"生成候选阶梯"；阶梯按得分降序列出各份不同正文，
  标注与上一阶的首异字节与穷尽状态；点击任一阶即在既有十六进制视图与片段见证高亮中联动切换。
  级数一变立即撤下旧阶梯并按新级数重取；IMPOSSIBLE 时阶梯为空并明确已穷尽。
...
**候选阶梯**：任一可还原正文的最优覆盖恰为"全部与该正文逐字节一致的片段"
（权重为正，相容片段全取只增不减），即冲突图上覆盖全集的**极大独立集**，
且不同正文与此类极大独立集一一对应。阶梯求解用带"必选剪枝"的 DFS 枚举极大独立集
（排除分支仅当存在更靠后的相容冲突邻居时才可能极大，否则该片段必选），
不重不漏地收集每份不同正文——同一正文的海量重复覆盖只汇入同一极大集。
随后按（总权重，片段数）降序、同分按无符号正文升序取前 N 阶，
并为每阶标注与上一阶首次不同的字节位置。

求解器已通过 300 组随机小实例与暴力枚举的交叉验证（零分歧），
阶梯另经 400 组随机实例与暴力枚举交叉验证（含见证、首异字节与穷尽标记），
28 片段对抗实例在毫秒级完成。
